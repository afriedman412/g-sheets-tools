...
# MAJOR CODE
class gSheet:
    def __init__(self, sheet_id: str = None, name: str = None, suffix: str = None):
        self.suffix = suffix
    ...
    def load_from_range(self, data_range, return_df=True, assume_headers=True):
        ...
        try:
            result = (
                self.sheet_object.values()
                .get(spreadsheetId=self.sheet_id, range=data_range)
                .execute()
            )
    ...
    def create_new_page(self, page_name):
        ...
        request_body = {
            "requests": [{"addSheet": {"properties": {"title": page_name}}}]
        }

        try:
            response = self.sheet_object.batchUpdate(
                spreadsheetId=self.sheet_id, body=request_body
            ).execute()
            self.load_page_info()
            return response
    ...
    def delete_page(self, page_name):
        ...
        request_body = {
            "requests": [
                {"deleteSheet": {"sheetId": self.page_info[page_name]["sheetId"]}}
            ]
        }
    ...
    def write_to_page(self, new_data: pd.DataFrame, data_range, write_setting="update"):
        ...
        value_range_body = {"range": data_range, "values": data_out_for_upload}
